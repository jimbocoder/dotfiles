...
fzf() {
  /usr/bin/ruby --disable-gems $(which fzf) "$@"
}
export -f fzf > /dev/null

# Auto-completion
# ---------------
[[ $- =~ i ]] && source /usr/local/Cellar/fzf/0.8.9/fzf-completion.bash
...
if [ -z "$(set -o | grep '^vi.*on')" ]; then
  ...
  bind '"\C-r": " \C-e\C-u$(HISTTIMEFORMAT= history | fzf +m -n2..,.. | sed \"s/ *[0-9]* *//\")\e\C-e\er"'
